...
from src.aplicacion.exceptions import ErrorProcesamientoWebhook

class ProcesarMensajeEntrante:
    def __init__(self, puerta_enlace_chatwoot: PuertaEnlaceChatwoot, puerta_enlace_rasa: PuertaEnlaceRasa, use_rasa: bool, logger: Registrador):
    ...
    async def ejecutar(self, mensaje: Mensaje) -> None:
        ...
        self.logger.informar(f'Procesando mensaje. Modo Rasa: {self.use_rasa}')
        try:
            if self.use_rasa:
    ...
    async def _enviar_a_chatwoot(self, conv_id: str, content: str) -> None:
        mensaje_respuesta = Mensaje.crear_seguro(
