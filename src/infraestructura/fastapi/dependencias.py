...
from src.infraestructura.asr.asr_gateway import WhisperASRGateway
from src.infraestructura.settings.config import ajustes
from src.infraestructura.settings.registrador import logger

@lru_cache()
def obtener_cliente_http() -> ClienteHTTP:
    return cast(ClienteHTTP, ClienteHttpx())
...
@lru_cache()
def obtener_puerta_enlace_asr() -> WhisperASRGateway:
    # Reutilizamos token de Chatwoot para acceder a archivos
    return WhisperASRGateway(
        cliente_http=obtener_cliente_http(),
        api_token=ajustes.token_api_chatwoot
    )
